...
void rsa_compute_wrapper(uint32 encryption, uint32 padding, uint32 pubpriv, uint32 n_bytes, uint32 e_bytes, uint32 in_bytes, rsa_plm_r_t &r, rsa_plm_n_t &n, rsa_plm_e_t &e, rsa_plm_in_t &in, rsa_plm_out_t &out) {
    ...
    ESP_REPORT_INFO(VON, "rsa_pubpriv: %u", ESP_TO_UINT32(pubpriv));
    ESP_REPORT_INFO(VON, "rsa_n_bytes: %u", ESP_TO_UINT32(n_bytes));
    ESP_REPORT_INFO(VON, "rsa_e_bytes: %u", ESP_TO_UINT32(e_bytes));
    ESP_REPORT_INFO(VON, "rsa_in_bytes: %u", ESP_TO_UINT32(in_bytes));
...
void CCS_BLOCK(crypto_cxx_catapult)(
    ...
    uint32 rsa_encryption = 0;
    uint32 rsa_padding = 0;
    uint32 rsa_pubpriv = 0;
    uint32 rsa_n_bytes = 0;
    uint32 rsa_e_bytes = 0;
    uint32 rsa_in_bytes = 0;
...
AES_STORE_LOOP:
    ...
    if (crypto_algo == CRYPTO_RSA_MODE) {
        ...
        ESP_REPORT_INFO(VOFF, "conf_info.rsa_in_bytes = %u", ESP_TO_UINT32(rsa_in_bytes));
        ESP_REPORT_INFO(VOFF, "conf_info.rsa_e_bytes = %u", ESP_TO_UINT32(rsa_e_bytes));
        ESP_REPORT_INFO(VOFF, "conf_info.rsa_n_bytes = %u", ESP_TO_UINT32(rsa_n_bytes));
        ESP_REPORT_INFO(VOFF, "conf_info.rsa_pubpriv = %u", ESP_TO_UINT32(rsa_pubpriv));
        ESP_REPORT_INFO(VOFF, "conf_info.rsa_padding = %u", ESP_TO_UINT32(rsa_padding));
        ESP_REPORT_INFO(VOFF, "conf_info.rsa_encryption = %u", ESP_TO_UINT32(rsa_encryption));
        ...
        // - Do some math (ceil) to get the number of data and DMA words given in_bytes
        dma_read_data_index = 0;
        dma_read_data_length = (rsa_in_bytes + 4 - 1) / 4; // ceil(in_bytes / 4)
        dma_read_info = {(dma_read_data_index + 2 - 1) / 2, (dma_read_data_length + 2 - 1) / 2, SIZE_WORD}; // ceil(dma_read_data_legnth / 2)
        bool dma_read_ctrl_done = false;
RSA_LOAD_IN_CTRL_LOOP:
        do { dma_read_ctrl_done = dma_read_ctrl.nb_write(dma_read_info); } while (!dma_read_ctrl_done);
        ...
        if (dma_read_ctrl_done) { // Force serialization between DMA control and DATA data transfer
RSA_LOAD_IN_LOOP:
            for (uint16_t i = 0; i < RSA_PLM_IN_SIZE; i+=2) {
                ...
                data_0 = data_dma.template slc<WL>(WL*0).to_uint();
                data_1 = data_dma.template slc<WL>(WL*1).to_uint();
                rsa_plm_in.data[i+0] = data_0;
                rsa_plm_in.data[i+1] = data_1;
        ...
        dma_read_data_index = (rsa_in_bytes + 4 - 1) / 4;
        dma_read_data_length = (rsa_e_bytes + 4 - 1) / 4; // ceil(e_bytes / 4)
        dma_read_info = {(dma_read_data_index + 2 - 1) / 2, (dma_read_data_length + 2 - 1) / 2, SIZE_WORD}; // ceil(dma_read_data_legnth / 2)
        dma_read_ctrl_done = false;
RSA_LOAD_E_CTRL_LOOP:
        do { dma_read_ctrl_done = dma_read_ctrl.nb_write(dma_read_info); } while (!dma_read_ctrl_done);
        ...
        if (dma_read_ctrl_done) { // Force serialization between DMA control and DATA data transfer
RSA_LOAD_E_LOOP:
            for (uint16_t i = 0; i < RSA_PLM_E_SIZE; i+=2) {
                ...
                data_0 = data_dma.template slc<WL>(WL*0).to_uint();
                data_1 = data_dma.template slc<WL>(WL*1).to_uint();
                rsa_plm_e.data[i+0] = data_0;
                rsa_plm_e.data[i+1] = data_1;
        ...
        dma_read_data_index = (rsa_in_bytes + 4 - 1) / 4 + (rsa_e_bytes + 4 - 1) / 4;
        dma_read_data_length = (rsa_n_bytes + 4 - 1) / 4; // ceil(n_bytes / 4)
        dma_read_info = {(dma_read_data_index + 2 - 1) / 2, (dma_read_data_length + 2 - 1) / 2, SIZE_WORD}; // ceil(dma_read_data_legnth / 2)
        dma_read_ctrl_done = false;
RSA_LOAD_N_CTRL_LOOP:
        do { dma_read_ctrl_done = dma_read_ctrl.nb_write(dma_read_info); } while (!dma_read_ctrl_done);
        ...
        if (dma_read_ctrl_done) { // Force serialization between DMA control and DATA data transfer
RSA_LOAD_N_LOOP:
            for (uint16_t i = 0; i < RSA_PLM_N_SIZE; i+=2) {
                ...
                data_0 = data_dma.template slc<WL>(WL*0).to_uint();
                data_1 = data_dma.template slc<WL>(WL*1).to_uint();
                rsa_plm_n.data[i+0] = data_0;
                rsa_plm_n.data[i+1] = data_1;
        ...
        dma_read_data_index = (rsa_in_bytes + 4 - 1) / 4 + (rsa_e_bytes + 4 - 1) / 4 + (rsa_n_bytes + 4 - 1) / 4;
        dma_read_data_length = (rsa_n_bytes + 4 - 1) / 4; // ceil(r_bytes / 4)
        dma_read_info = {(dma_read_data_index + 2 - 1) / 2, (dma_read_data_length + 2 - 1) / 2, SIZE_WORD}; // ceil(dma_read_data_legnth / 2)
        dma_read_ctrl_done = false;
RSA_LOAD_R_CTRL_LOOP:
        do { dma_read_ctrl_done = dma_read_ctrl.nb_write(dma_read_info); } while (!dma_read_ctrl_done);
        ...
        if (dma_read_ctrl_done) { // Force serialization between DMA control and DATA data transfer
RSA_LOAD_R_LOOP:
            for (uint16_t i = 0; i < RSA_PLM_R_SIZE; i+=2) {
                ...
                data_0 = data_dma.template slc<WL>(WL*0).to_uint();
                data_1 = data_dma.template slc<WL>(WL*1).to_uint();
                rsa_plm_r.data[i+0] = data_0;
                rsa_plm_r.data[i+1] = data_1;
        ...
        rsa_compute_wrapper(rsa_encryption, rsa_padding, rsa_pubpriv, rsa_n_bytes, rsa_e_bytes, rsa_in_bytes, rsa_plm_r, rsa_plm_n, rsa_plm_e, rsa_plm_in, rsa_plm_out);
        ...
        // - Each DMA transaction is 2 output words
        // - Do some math (ceil) to get the number of data and DMA words given out_bytes
        dma_write_data_index = (rsa_in_bytes + 4 - 1) / 4 + (rsa_e_bytes + 4 - 1) / 4 + (rsa_n_bytes + 4 - 1) / 4 + (rsa_n_bytes + 4 - 1) / 4;
        dma_write_data_length = (rsa_out_bytes + 4 - 1) / 4; // ceil(out_bytes / 4)
        dma_write_info = {(dma_write_data_index + 2 - 1) / 2, (dma_write_data_length + 2 - 1) / 2, SIZE_WORD};
        bool dma_write_ctrl_done = false;
RSA_STORE_CTRL_LOOP:
        do { dma_write_ctrl_done = dma_write_ctrl.nb_write(dma_write_info); } while (!dma_write_ctrl_done);
        ...
        if (dma_write_ctrl_done) { // Force serialization between DMA control and DATA data transfer
RSA_STORE_LOOP:
            for (uint16_t i = 0; i < RSA_PLM_OUT_SIZE; i+=2) {
                ...
                data_t data_0(rsa_plm_out.data[i+0]);
                //data_t data_1((i+1 >= dma_write_data_length)?ZERO:plm_out.data[i+1]);
                data_t data_1(rsa_plm_out.data[i+1]);
